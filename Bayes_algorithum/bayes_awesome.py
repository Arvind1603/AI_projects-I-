...
class SimpleSampler(object):
    ...
    def get_prob(self, query_vals, num_samples):
        ...
        for i in range(num_samples):
            ...
            temp = 0
            for j in query_vals:
                if sample[j] == query_vals[j]:
                    temp += 1
            if temp == len(query_vals):
                count += 1
    ...
    def get_prob(self, query_vals, evidence_vals, num_samples):
        ...
        for i in range(num_samples):
            ...
            if (len(evidence_vals)==0):
                temp = 0
                for a, b in query_vals.items():
                    if (sample[a] == b):
                        temp += 1
                    if (temp == len(a)):
                        q_count +=1
            elif (len(evidence_vals) != 0):
                temp = 0
                for a, b in evidence_vals.items():
                    if (sample[a] == b):
                        temp += 1
                    if (temp == len(a)):
                        e_count += 1
                ...
                for a, b in query_vals.items():
                    ...
                    if (temp == len(a)):
                        q_count += 1
        if (len(evidence_vals)==0):
            return q_count/num_samples
        return q_count/e_count
...
class LikelihoodWeightingSampler(SimpleSampler):
    ...
    def get_prob(self, query_vals, evidence_vals, num_samples):
        ...
        for i in range(num_samples):
            sample, wei = self.generate_sample(evidence_vals)
            total += wei 
...
def bayes_sample_size_plot(sampler1, sampler2, query, evidence, label1, label2, title, fname):
    ...
    query = { 'A': True }
    evidence = { 'P': False, 'E': False }

    sampler_reject = RejectionSampler(awesome_nodes)
    sampler_like = LikelihoodWeightingSampler(awesome_nodes)

    l = list(range(50,10000,20))

    reject_y = [sampler_reject.get_prob(query, evidence, n) for n in l]
    like_y = [sampler_like.get_prob(query, evidence, n) for n in l]

    two_line_plot(l, reject_y, "Rejection", l, like_y, "Likelihood", "Rejection vs Likelihood","bayes_awesome.pdf")
