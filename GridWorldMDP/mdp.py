...
def value_iteration(mdp, gamma, epsilon):
    ...
    while True:
        ...
        for j in states:
            ...
            for k in A:
                util_of_state = 0
                nextstates = mdp.get_successor_probs(j,k)
                for move, prob in nextstates.items():
                    ex_util_s = prob * U[move]
                    util_of_state += ex_util_s
                mainA.append(util_of_state)

            Result = mdp.get_reward(j) + gamma*max(mainA)

            mainU[j] = Result


        temp = 0
        for a in mdp.get_states():
            temp1 = mainU[a] - U[a]
            if temp1 > temp:
                temp = temp1
